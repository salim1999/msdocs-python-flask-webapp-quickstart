...
class Chatbot:
    ...
    def main(self, question, args=None):
        """
        ...
        if not self.args.test:
            self.args.test = "interactive"  # Use the current working directory

        if not self.args.rootDir:
            self.args.rootDir = os.getcwd()  # Use the current working directory
            print(self.args.rootDir)
        ...
        return self.mainTestInteractive(self.sess, question)
    ...
    def mainTestInteractive(self,  sess, question):
        ...
        answer = '{}{}'.format(self.SENTENCES_PREFIX[1], self.textData.sequence2str(answer, clean=True))
        print(answer)
        return answer

        path = os.getcwd().replace('.', '') + '/Response.txt'
